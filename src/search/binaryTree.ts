...
function createTree<T>(root: Node<T>|null|undefined, value: T): Node<T> {
  ...
  if (value < root.value) { root.left = createTree(root.left, value); }
  if (value > root.value) { root.right = createTree(root.right, value); }
...
function balanceNode<T>(root: Node<T>|null|undefined): number {
  ...
  if (difference < -1) { root = leftRotation(root); }
  if (difference > 1) { root = rightRotation(root); }
...
export function getSubTree<T>(root: Node<T>|null|undefined, callback: SearchCB<T>): Node<T>|null {
  ...
  if (callback(root.value) === -1) { return getSubTree(root.left, callback); }
  if (callback(root.value) === 1) { return getSubTree(root.right, callback); }
